...
Set the password on the **agent side** (not the target side):

```bash
# Generic — works for any target using EDR_WD_TARGET_PASSWORD
export EDR_WD_TARGET_PASSWORD='<YOUR_PASSWORD>'

# Or per-target env var (if config uses a target-specific env var name)
export EDR_WD_WIN_DEV_PASSWORD='<YOUR_PASSWORD>'
```

```powershell
# Windows PowerShell (agent side)
$env:EDR_WD_TARGET_PASSWORD = "<YOUR_PASSWORD>"
```

Password is read by `agent/ssh_runner.py` via `os.environ.get(password_env)` and never written to logs, config files, or commit history.
...
Flow: launch HisecEndpointAgent → connect → click edrWidget → EDRClient popup appears → connect EDRClient.
...
| `EDR_WD_TARGET_PASSWORD` | from config | SSH password for any target (agent-side env, not target-side) |
...
|----------|-----------------|-------|
| Linux / Mac | password or key | password uses `sshpass`; key uses OpenSSH |
| Windows | **password** or key | password uses Paramiko (no `sshpass` needed); key uses OpenSSH |

On Windows agent, password auth uses Paramiko (pure Python) — no external tools needed.
