"""Dump HiSec Endpoint window tree - by hwnd directly"""
import json
import win32gui
import win32con
import win32api
...
def get_control_tree(hwnd, depth=0, max_depth=20):
    if depth > max_depth:
        return []
    results = []
    try:
    ...
    def enum_child(hwnd, lparam):
        if hwnd != 0:
            results.extend(get_control_tree(hwnd, depth+1, max_depth))
        return True
    try:
